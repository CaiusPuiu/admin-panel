...
class App extends React.Component {
  ...
  handleUserChange() {
    if (this.state.usersTrigger === false) {
      this.setState((state) => ({
        usersTrigger: !this.state.usersTrigger,
        postsTrigger: !this.state.postsTrigger,
      }));
    } else {
      this.setState((state) => ({
        usersTrigger: this.state.usersTrigger,
    ...
    fetch("https://jsonplaceholder.typicode.com/users")
      ...
      .then((usersJson) => {
        ...
        filteredJson.forEach((user) => {
          ...
          if (user.id % 2 === 0) {
            user.picture =
              "https://deadline.com/wp-content/uploads/2019/10/shutterstock_editorial_10434333bm.jpg";
          } else if (user.id % 3 === 0) {
            user.picture =
              "https://www.whitehouse.gov/wp-content/uploads/2021/01/joe_hero.jpg";
          } else {
            user.picture =
              "https://e00-marca.uecdn.es/assets/multimedia/imagenes/2021/01/23/16114264444946.jpg";
          }
        });
    ...
    return (
      ...
      >
        ...
        <div className="edit-panel">
          ...
          <input
            type="color"
            onChange={(event) => this.hadleColorChange(event)}
          />

          <input
            type="button"
            value="Afiseaza utilizatori"
            onClick={this.handleUserChange}
          />
          <input
            type="button"
            value="Afiseaza postari"
            onClick={this.handlePostsChange}
          />
          <input
            type="button"
